...
void connect() {
  ...
  Serial.print("\nconnecting...");
  while (!client.connect("Kitchen","tugasiot","9SVwxjXIVUW5YXdx")) {
    Serial.print(".");
    delay(1000);
  }
  ...
  client.subscribe("control/kitchen/led");
...
void loop() {
 ...
 if ((millis() - timerDHT) > WAKTUDHT) {
   ...
   delay(2000);
    ...
    client.publish("control/kitchen/suhu", msg, true,1); //publish message suhu
    snprintf (msg, MSG_DHT, "%s", itoa(kelembaban,msg,10)); //ubah nilai int ke string
    Serial.print("Kelembaban : ");
    Serial.println(kelembaban);
    client.publish("control/kitchen/kelembaban", msg, true,1); //publish message kelembaban
