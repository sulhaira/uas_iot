...
void connect() {
  ...
  // pj4go30PoL199y0P
  Serial.print("\nconnecting...");
   while (!client.connect("bedroom","tugasiot","9SVwxjXIVUW5YXdx")) {
  //while (!client.connect("Bedroom","iotmulawarman2023","pj4go30PoL199y0P")) {
    Serial.print(".");
    delay(1000);
  }
  ...
  client.subscribe("control/bedroom/led");
...
void loop() {
 ...
 if ((millis() - timerDHT) > WAKTUDHT) {
   ...
   delay(2000);
    ...
    client.publish("control/bedroom/suhu", msg, true,1); //publish message suhu
    snprintf (msg, MSG_DHT, "%s", itoa(kelembaban,msg,10)); //ubah nilai int ke string
    Serial.print("Kelembaban : ");
    Serial.println(kelembaban);
    client.publish("control/bedroom/kelembaban", msg, true,1); //publish message kelembaban
